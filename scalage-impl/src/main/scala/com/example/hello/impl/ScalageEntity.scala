...
/**
  * This is an event sourced entity. It has a state, [[ScalageState]], which
  ...
  */
class ScalageEntity extends PersistentEntity {

  override type Command = ScalageCommand[_]
  override type Event = ScalageEvent
  override type State = ScalageState

  /**
    * The initial state. This is used if there is no snapshotted state to be found.
    */
  override def initialState: ScalageState = ScalageState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case ScalageState(message, _) => Actions().onCommand[UseGreetingMessage, Done] {
      ...
      case (GreetingMessageChanged(newMessage), state) =>
        ...
        // the event.
        ScalageState(newMessage, LocalDateTime.now().toString)
...
/**
  ...
  */
case class ScalageState(message: String, timestamp: String)
...
object ScalageState {
  /**
    ...
    */
  implicit val format: Format[ScalageState] = Json.format
}

/**
  * This interface defines all the events that the ScalageEntity supports.
  */
sealed trait ScalageEvent extends AggregateEvent[ScalageEvent] {
  def aggregateTag = ScalageEvent.Tag
}

object ScalageEvent {
  val Tag = AggregateEventTag[ScalageEvent]
}

/**
  * An event that represents a change in greeting message.
  */
case class GreetingMessageChanged(message: String) extends ScalageEvent
...
/**
  ...
  */
sealed trait ScalageCommand[R] extends ReplyType[R]
...
/**
  ...
  */
case class UseGreetingMessage(message: String) extends ScalageCommand[Done]
...
/**
  ...
  * person.
  */
case class Hello(name: String) extends ScalageCommand[String]
...
/**
  ...
  * application loader.
  */
object ScalageSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[ScalageState]
  )
}
